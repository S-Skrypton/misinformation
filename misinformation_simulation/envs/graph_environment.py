...
def create_social_network(num_nodes):
    ...
    G = nx.DiGraph()
    num_celebrities = min(int(0.004 * num_nodes), 50)
    num_robots = min(int(0.05 * num_nodes), 50)
    ...
    for i in range(num_nodes):
        user_type = types[i]
        raw_repost_probability = 0.3 if user_type == 2 else 0.05 if user_type == 0 else 0.01
    ...
    for i in G.nodes():
        user_data = G.nodes[i]
        followers = random.sample(list(G.nodes), min(int(random.uniform(0.1, 0.2) * num_nodes), 1) 
                                  if user_data['type'] == 2 else min(int(random.uniform(0, 0.005) * num_nodes), 50))
        followings = random.sample([n for n in G.nodes if n != i], min(int(random.uniform(0, 0.005) * num_nodes), 10) 
...
def apply_action(node_id, action, G):
    ...
    if action == 0:
        ...
        G.nodes[node_id]['adjust_rpp'] *= 0.9
    elif action == 2:
...
def compute_reward(current_node, next_node, action, G):
    ...
    num_followers = len(G.nodes[current_node]['followers'])
    action_cost = cost_of_action(action,num_followers)
    current_node_type_cost = cost_of_node_type(G.nodes[current_node]['type'],num_followers)
    # For root node
    next_node_type_cost = 0
    if next_node is not None:    
        next_node_type_cost = cost_of_node_type(G.nodes[next_node]['type'],num_followers)
